...
\(G(n+1)\).  We also prove that the directed rays from any two vertices
eventually meet.  Thus the underlying undirected graph of \(\mathcal C\) is
a graded tree.  Consequently, any two triangulations of \(M\) admit a
Pachner sequence in vertex-normal form: first facet subdivisions, then only
...
\(G(n)\) be the graph whose vertices are the \(n\)-vertex simplicial
triangulations of \(M\), with an edge for each vertex-preserving bistellar
move.  Write
\[
...
but they cannot split.  The resulting normal form for Pachner paths is worth
stating separately.

\begin{theorem*}[Normal form for Pachner paths]
...
Pachner sequence can be reordered in this way.  It is also distinct from the
unimodal sequences of Burton and He \cite{BH26}.  Their theorem concerns
generalized triangulations of \(3\)-manifolds, in which tetrahedra are glued
along faces, self-identifications are allowed, and the triangulation has a
single vertex.  Complexity is graded by the number of tetrahedra, and the
permitted local moves include, besides the exchanges of two tetrahedra for
three and of three for two, additional moves that delete a pair of tetrahedra
and have no counterpart for simplicial complexes.  Here the triangulations
are simplicial, the horizontal slices fix an arbitrary vertex count, and the
grading itself is by vertex number.  The two normal-form statements concern
...
examples \cite{GS67}.  Both lie in \(\mathcal T(8)\): each is one \(2\)--\(3\) flip from a
polytopal triangulation or from the other, so
\(G(8)=\mathcal T(8)\).

At \(n=9\), Altshuler, Bokowski, and Steinberg \cite{ABS80} showed that
\(G(9)\) is connected.  Using their classification together with
Firsching's polytopality data \cite{F20}, we verified that
\(G(9)\) has \(1296\) vertices, of which \(1142\) are polytopal
and \(154\) are non-polytopal, and that the induced subgraph on the
non-polytopal vertices is itself connected.  Hence
\(G(9)=\mathcal T(9)\).
...
four can encounter substantial combinatorial depth \cite{BBS25}.  What are
...
\bibitem{BBS25}
R. A. Burke, B. A. Burton, and J. Spreer.
``Small triangulations of 4-manifolds and the 4-manifold census.''
In \emph{41st International Symposium on Computational Geometry (SoCG
2025)}, LIPIcs 332, 28:1--28:16, 2025.
